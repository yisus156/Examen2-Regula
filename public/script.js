...
socket.on('msg', function(data){
  console.log(data);
  if(data == 2){
    socket.emit('respuesta',9);
    if(canvas.width - (tankX+tankWidth) > tankdX) {
      tankX+=tankdX;
    } 
  }
  if(data == 4){
    socket.emit('respuesta',8);
    if (tankX-tankdX>0) {
      tankX-=tankdX;
    }
  }
...
function moveInvaderBullets(){
  for(let i = 0 ; i < invaderBulletsArray.length; i++){
    ...
    )
    {
      ...
      if(hasLifeDecreased=true){
        socket.emit('respuesta',11);
      }
    }
